...
class AllNotesFragment : Fragment(R.layout.fragment_all_notes) {
    ...
    private fun subscribeToNotes() = lifecycleScope.launch {
        /*
        * to get all note from db by collect coroutine flow
        * */
    ...
    val itemTouchHelperCallback = object : ItemTouchHelper.SimpleCallback(
        ...
        override fun onChildDraw(
            ...
            super.onChildDraw(
                ...
                recyclerView,
                viewHolder,
                dX / 2 /* to swipe item half screen to delete*/,
                dY,
                actionState,
                isCurrentlyActive
            )
    ...
    private fun searchNotes(query: String) = lifecycleScope.launch {
        ...
        noteAdapter.notes = noteViewModel.notes.first().filter {
            it.noteTitle?.contains(query, true) == true || it.description?.contains(
                query,
                true
            ) == true
        }
